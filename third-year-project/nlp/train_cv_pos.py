...
pos_dict = {i: [] for i in range(1, 11)}
...
for j in range(3):
    ...
    for train_split, test_split in get_splits(ids, 5):
        ...
        X_train = train_preprocess.get_album_frequency(X_train)
        X_test = test_preprocess.get_album_frequency(X_test)
        ...
        # Fit the encoder
        drop_tags = [x * "p" + "tag" for x in range(j + 1, 3)] + [x * "n" + "tag" for x in range(j + 1, 3)]

        X_train = X_train.drop(columns=drop_tags)
        X_test = X_test.drop(columns=drop_tags)

        tags = ["tag"] + [x * "p" + "tag" for x in range(1, j + 1)] + [x * "n" + "tag" for x in range(1, j + 1)]
        enc.fit(X_train.loc[:, tags + ["Platform"]])
        # Encode any categorical variables
        one_hot = enc.transform(X_train.loc[:, tags + ["Platform"]]).toarray()
        # Transform the data
        ...
        X_train = pd.concat([X_train, one_hot], axis=1).drop(
            columns=tags + ["Text", "Platform", "Artist", "Album",
                     "Word id", "Word_Sentence id", "Sentence id"])
        features = X_train.columns

        # Fit the encoder
        enc.fit(X_test.loc[:, tags + ["Platform"]])
        # Encode any categorical variables
        one_hot = enc.transform(X_test.loc[:, tags + ["Platform"]]).toarray()
        # Transform the data
        one_hot = pd.DataFrame(one_hot, columns=enc.get_feature_names_out())
        # Replace the columns with their one-hot variants
        X_test = pd.concat([X_test, one_hot], axis=1).drop(
            columns=tags + ["Text", "Platform", "Artist", "Album", "Word id",
                     "Word_Sentence id",
                     "Sentence id"])
    ...
    pos_df = pos_df.append(lift_df.mean(axis=0), ignore_index=True)

pos_df.to_csv("C:\\Users\\tommy\\OneDrive\\University\\Year 3\\Third Year Project\\Platform Album Data\\Model "
               "Evaluation\\pos_lift.csv")
